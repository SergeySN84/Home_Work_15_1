...
from src.category import Category  # или from src import Category
...
def test_category_products_property_empty():
    """Проверка products для пустой категории"""
    category = Category("Категория", "Описание", [])
    assert category.products == ""


def test_category_init_with_products():
    """Проверка, что продукты из списка добавляются при создании категории"""
...
def test_category_total_categories_counter():
    """Проверка счётчика category_count"""
...
def test_category_products_order():
    ...
    product1 = Product("A", "Описание", 100.0, 1)
    product2 = Product("B", "Описание", 100.0, 1)
    category = Category("Категория", "Описание", [product1, product2])
    expected = "A, 100.0 руб. Остаток: 1 шт.\nB, 100.0 руб. Остаток: 1 шт."
    assert category.products == expected


def test_category_access_private_products():
    """Проверка доступа к приватному списку через name mangling"""
    category = Category("Категория", "Описание", [])
    product = Product("Товар", "Описание", 100.0, 5)
    category.add_product(product)
